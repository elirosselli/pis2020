...
import { REQUEST_TYPES, MUTEX_NAMES } from '../utils/constants';
import ERRORS from '../utils/errors';
import { initializeErrors, fetch } from '../utils/helpers';

const getTokenOrRefresh = type => {
  // Se crea una instancia del mutex runner para prevenir
  // que la función getTokenOrRefresh pueda ser llamada cuando existe
  // una ejecución de la función en curso.
  const MutexRunner = useMutex();
  const mutex = new MutexRunner(MUTEX_NAMES.getTokenOrRefresh);
  ...
  mutex.run(async () => {
    ...
    try {
      ...
      if (
        type === REQUEST_TYPES.GET_REFRESH_TOKEN &&
      ...
      // Se desboquea el mutex una vez que se termina la llamada.
      mutex.unlock();
